...
FVector USixDOFMovementComponent::GetUpdatedLinearVelocity(const FVector &Location, const FQuat &Rotation, const FVector &OldVelocity, const FVector &Input, float DeltaTime) const {
...
FVector USixDOFMovementComponent::GetUpdatedAngularVelocity(const FQuat &Rotation,const FVector &OldAngularVelocity,const FVector &Input, float DeltaTime) const {
	FVector NewInput = Input;
...
float USixDOFMovementComponent::AxisAcceleration(EAccelerationMode Mode, float Current, float Goal, float MaxSpeed, float Inertia, float Input, float DeltaTime) const {
	...
	case(EAccelerationMode::AM_FrictionlessClamped): {
		if (Inertia > 0.0f) {
			return FMath::Clamp((Current + Input * MaxSpeed * DeltaTime / Inertia),-MaxSpeed,MaxSpeed);
		}
		else {
			UE_LOG(LogTemp, Warning, TEXT("FrictionlessClamped requires positive inertia"));
			return Current;
		}
	}

	case(EAccelerationMode::AM_Custom): {
		return GetCustomAxisAcceleration(Current, Goal, MaxSpeed, Inertia, Input, DeltaTime);
	}
