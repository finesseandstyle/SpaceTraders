...
USixDOFMovementComponent::USixDOFMovementComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer){
	...
	SetIsReplicatedByDefault(ReplicateInputs || ReplicateMovement);
...
void USixDOFMovementComponent::BeginPlay() {
	Super::BeginPlay();

	if (PauseReplicationWhenStationary && IsValid(GetOwner()) ) {
		PreviousLocation = GetOwner()->GetActorLocation();
		PreviousRotation = GetOwner()->GetActorRotation();
		TimeSinceMovement = StationaryTimeToSleep;
	}
}
...
void USixDOFMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction) {
	...
	BroadcastClientMovement(DeltaTime);
